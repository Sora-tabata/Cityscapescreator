    ...
    def viz(self):
        ...
        for camera in self.cameras:
            ...
            if image is not None:  # Check if image is not None
                filename = self.out_path + str(camera.serial_number) + "_" + str(self.count) + ".png"
                cv2.imwrite(filename, image)
                camera_images.append(image)
                self.count = self.count+1

        if len(camera_images) > 0:  # Check if the list is not empty
...
try:
    ...
    while True:
        ...
        if elapsed_time > 10:  # Check if 10 seconds have passed
            print("10 seconds have passed. Stopping recording.")
            break

        recorder.viz()  # Continue to visualize the camera feed
