...
#
# Usage: json2labelImg.py [OPTIONS] <path to the labels directory>
# Options:
...
def json2labelImg(labels_dir,encoding="ids"):
    annotation = Annotation()
    os.chdir( labels_dir )
    print("Working on the directory: "+os.getcwd())
    for file in glob.glob("*.json"):
    	name = os.path.splitext(file)[0]
    	annotation.fromJsonFile(name+".json")
    	labelImg   = createLabelImage( annotation , encoding )
    	if encoding == "ids":
    		labelImg.save( name+"_labelIds.png" )
    	elif encoding == "trainIds":
    		labelImg.save( name+"_labelTrainIds.png" )
    	elif encoding == "color":
    		labelImg.save( name+"_color.png" )
...
def main(argv):
    ...
    if len(args) == 0:
        printError( "Missing labels directory path" )
    elif len(args) > 1:
