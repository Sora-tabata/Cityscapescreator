...
def process_directory(json_directory, output_directory, encoding="ids"):
    for filename in os.listdir(json_directory):
        if filename.endswith('.json'):
            inJson = os.path.join(json_directory, filename)
            outImg = os.path.join(output_directory, f"{os.path.splitext(filename)[0]}_instanceIds.png")
            json2instanceImg(inJson, outImg, encoding)
